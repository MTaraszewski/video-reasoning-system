...
## 4. Selected — a three-tier set

Each tier answers a different question. Together they cover calibration, defensible
real-world performance, and the domain the brief's examples come from.

| Tier | Source | Ground truth | What ships in the repo | Answers |
|---|---|---|---|---|
| **1 — Synthetic** | Generated by us | **Exact**, zero labelling error | Generator + clips + labels | What is the model's *precision floor*, with labelling error removed? |
| **2 — Real, redistributable** | MEVA | Hand-labelled, cross-checked against MEVA's own annotations | Fetch script + our labels | Does it work on real fixed-camera footage? |
| **3 — Real, domain-relevant** | Roboflow `supervision` assets | Hand-labelled | Fetch script + our labels | Does it work on the industrial/retail framing the brief's examples describe? |

**Why tier 1 is not optional.** The capability probe measures how precisely the
model can place an event boundary. That number is meaningless if the reference
labels are themselves fuzzy by ±0.3 s. Only constructed ground truth separates the
model's error from the labeller's — and every tier-2 and tier-3 result is read
against it.
...
would close the forklift gap noted in §9.
...
**Status: open decision, not ruled out.** The original rejection rested on three
reasons, and only the weakest survives. What remains is a judgement about whether a
non-reproducible tier is worth the domain coverage it buys.
...
- [ ] Decide the split between the three tiers within the 5–10 clip budget
